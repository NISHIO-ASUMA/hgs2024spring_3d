...
// マクロ定義
//***************************
#define PLAYER_JUMPHEIGTH (15.0f)	// プレイヤーのジャンプ量
#define PLAYER_GRAVITY (0.7f)		// プレイヤーにかかる重力量
...
// 体力バーのテクスチャ設定
//*******************************
const char* LIFEBAR_TEX[LIFE_MAX] =
{
	"data\\TEXTURE\\ber.png",			// 体力バー
	"data\\TEXTURE\\lifebar.png",		// 体力バーのフレーム
};
...
void InitPlayer(void)
{
	...
	g_Player.nIdxShadow = SetShadow(D3DXVECTOR3(g_Player.pos.x,0.0f,g_Player.pos.z), g_Player.rot,D3DXVECTOR3 (1.0f,0.0f,1.0f));// 影のインデックス
...
void UpdatePlayer(void)
{
	...
	{// ジャンプしていなくて着地していないとき
		if (KeyboardTrigger(DIK_SPACE) == true || JoypadTrigger(JOYKEY_A) == true)
		{
			...
			// ジャンプキーが押された
			g_Player.move.y = PLAYER_JUMPHEIGTH;
		}
	}
	...
	// 重力
	g_Player.move.y -= PLAYER_GRAVITY;		// 重力加算
	...
	// 当たり判定関数
	CollisionBlockToPlayer3D(&g_Player,&g_Player.pos, &g_Player.posOld, &g_Player.move, &g_Player.size, GetInfoModel(), GetBlock());
